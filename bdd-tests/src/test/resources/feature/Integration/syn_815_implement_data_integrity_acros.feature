Feature: Implement data integrity across BigQuery marts, dbt transformations, and the   Neo4j graph (SYN-815)
  Source: Jira ticket SYN-815

  Background:
    Given the QE Quality Agent is reachable

  @JiraGenerated @SYN-815 @SYN-816
  ...
  @JiraGenerated @SYN-815 @SYN-817
  Scenario: AC2 - (dbt) When address_enriched is built by dbt, Then full_address must equal line1,
    When I run the BigQuery check:
      """
      SELECT COUNTIF(full_address != CONCAT(line1, ", ", city, ", ", postcode, ", ", country)) AS violations FROM `project-61358164-b71e-4422-a5c.qe_hack_syndicate_insight.address_enriched`
      """
    Then the result column "violations" should be 0

  @JiraGenerated @SYN-815 @SYN-818
