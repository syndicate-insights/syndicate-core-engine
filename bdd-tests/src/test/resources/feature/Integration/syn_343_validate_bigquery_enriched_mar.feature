Feature: Validate BigQuery enriched marts are refreshed within 24 hours (SYN-343)
  Source: Jira ticket SYN-343

  Background:
    Given the QE Quality Agent is reachable

  @JiraGenerated @SYN-343 @SYN-344
  ...
  @JiraGenerated @SYN-343 @SYN-345
  Scenario: AC2 - When account_enriched is queried, Then the most recent processed_at timestamp mu
    When I run the BigQuery check:
      """
      SELECT COUNTIF(max_processed_at < TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 24 HOUR)) AS violations FROM (SELECT MAX(`project-61358164-b71e-4422-a5c.qe_hack_syndicate_insight.account_enriched`.`processed_at`) AS max_processed_at FROM `project-61358164-b71e-4422-a5c.qe_hack_syndicate_insight.account_enriched`)
      """
    Then the result column "violations" should be 0
